...
$ergebnis = get-aduser -SearchBase $ou -Properties memberof,mail,description,whenCreated -filter {(ObjectClass -eq "user") -and (Name -notlike "DiscoverySearchMailbox*") -and (Enabled -eq $true)} | Where-Object {$group.DistinguishedName -notin $_.memberof -and $group2.DistinguishedName -notin $_.memberof -and $_.distinguishedName -like "*Buero*"} | Where-Object {$group3.DistinguishedName -in $_.memberof -or $group4.DistinguishedName -in $_.memberof} | ft -AutoSize Name,Mail,Description,whenCreated
$ergebnis
Echo "Summe:"
($ergebnis).Count - 4
Echo ""
